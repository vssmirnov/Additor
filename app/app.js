define(['require', 'Overtone', 'Envelope', 'ChannelStrip', 'AdditiveSynthVoice', 'AdditiveSynth', 'util', 'LiveKeyboard', 'EnvelopeGraph', 'LiveDial', 'LiveSlider', 'Histogram', 'LiveMeter', 'LiveDropMenu', 'StereoFeedbackDelay', 'DragNumberbox'], function (require, Overtone, Envelope, ChannelStrip, AdditiveSynthVoice, AdditiveSynth, util, LiveKeyboard, EnvelopeGraph, LiveDial, LiveSlider, Histogram, LiveMeter, LiveDropMenu, StereoFeedbackDelay, DragNumberbox) {
  'use strict';
  ...
  var app = function app() {
    ...
    // controllers
    var adt = {
      synth: { // additive synth controller
        node: {}
      },
      ot: { // overtone controller
      },
      env: { // envelope controller
        node: {}
      },
      filter: {
        node: {}
      ...
      voices: {} // voices controller
    };

    // envelope ctrl containers
    var otEnv_copyBtn = document.querySelector('#additor .env-ctrl .btn.copy');
    var otEnv_pasteBtn = document.querySelector('#additor .env-ctrl .btn.paste');
    ...
    adt.ot.histo = new Histogram({
      ...
      backgroundColor: '#111',
      barColor: '#f00'
    }).subscribe(this, function (overtoneAmplitudes) {
    ...
    for (var n = adt.synth.node.numOvertones - 1; n >= 0; n--) {
      var otAmp = n % 2 === 0 ? 1 / ((n + 1) / 2.2) : 1 / (n + 1);
    ...
    adt.env.graphSelectMenu = new LiveDropMenu({
      ...
      if (menuIndex === 0) {
        ...
        adt.env.ot.forEach(function (otEnv, otIndex) {
          ...
          otEnv.attackGraph.lineColor = otIndex === menuIndex ? 'hsla(' + otIndex * 23 % 360 + ', 50%, 50%, 1)' // selected for editing
          : 'hsla(' + otIndex * 23 % 360 + ', 50%, 50%, 0.15)'; // inactive
          otEnv.attackGraph.vertexColor = otIndex === menuIndex ? '#0f0' // selected for editing
          : 'hsla(' + otIndex * 23 % 360 + ', 50%, 50%, 0.15)'; // incative
          otEnv.sustainGraph.lineColor = otIndex === menuIndex ? 'hsla(' + otIndex * 23 % 360 + ', 50%, 50%, 1)' // selected for editing
          : 'hsla(' + otIndex * 23 % 360 + ', 50%, 50%, 0.15)'; // inactive
          otEnv.sustainGraph.vertexColor = otIndex === menuIndex ? '#0f0' // selected for editing
          : 'hsla(' + otIndex * 23 % 360 + ', 50%, 50%, 0.15)'; // inactive
          otEnv.releaseGraph.lineColor = otIndex === menuIndex ? 'hsla(' + otIndex * 23 % 360 + ', 50%, 50%, 1)' // selected for editing
          : 'hsla(' + otIndex * 23 % 360 + ', 50%, 50%, 0.15)'; // inactive
          otEnv.releaseGraph.vertexColor = otIndex === menuIndex ? '#0f0' // selected for editing
          : 'hsla(' + otIndex * 23 % 360 + ', 50%, 50%, 0.15)'; // inactive
    ...
    otEnv_copyBtn.addEventListener('mousedown', function () {
      clipboardOtEnvelope.attackEnvelope.vertices = overtoneEnvelopes[selectedOtIndex].attackEnvelope.vertices.map(function (vertex) {
        return vertex.slice();
      });
      clipboardOtEnvelope.sustainEnvelope.vertices = overtoneEnvelopes[selectedOtIndex].sustainEnvelope.vertices.map(function (vertex) {
        return vertex.slice();
      });
      clipboardOtEnvelope.releaseEnvelope.vertices = overtoneEnvelopes[selectedOtIndex].releaseEnvelope.vertices.map(function (vertex) {
        return vertex.slice();
      });
    });
    otEnv_pasteBtn.addEventListener('mousedown', function () {
      overtoneEnvelopes[selectedOtIndex].releaseEnvelope.vertices = clipboardOtEnvelope.releaseEnvelope.vertices.map(function (vertex) {
        return vertex.slice();
      });
      overtoneEnvelopes[selectedOtIndex].attackEnvelope.vertices = clipboardOtEnvelope.attackEnvelope.vertices.map(function (vertex) {
        return vertex.slice();
      });
      overtoneEnvelopes[selectedOtIndex].sustainEnvelope.vertices = clipboardOtEnvelope.sustainEnvelope.vertices.map(function (vertex) {
        return vertex.slice();
      });
    });
    ...
    adt.env.attackLengthNumbox = new DragNumberbox({
      ...
      appendString: ' ms',
      value: 1000
    }).subscribe(this, function (attackLengthNumboxVal) {});
    ...
    adt.voices.numVoicesNumbox.addEventListener('keyup', function (e) {
      if (e.key === 'Enter') {
        changeNumVoices();
      }
    });
    ...
    adt.voices.numOtNumbox.addEventListener('keyup', function (e) {
      if (e.key === 'Enter') {
        changeNumOvertones();
      }
    });
